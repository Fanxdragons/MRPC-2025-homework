                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  // v_dot = //?????
  v_dot = Eigen::Vector3d(0, 0, -g_) + 
          (R * Eigen::Vector3d(0, 0, thrust) - resistance * vnorm + external_force_) / mass_;
  ...
  omega_dot = J_.inverse() * 
              (moments + external_moment_ - cur_state.omega.cross(J_ * cur_state.omega));
